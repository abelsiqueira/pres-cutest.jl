...
\myframe{
  ...
  \begin{itemize}
    ...
    \item Selection of CUTEst problems.
  \end{itemize}
}
...
\begin{frame}[fragile]
  \ctr{Framework}
  \begin{itemize}
    \item Develop with \mintinline{julia}{Optimize.jl};
    \item Create the tests with \mintinline{julia}{NLPModels.jl};
    \item Easily access CUTEst with \mintinline{julia}{CUTEst.jl}.
    \item Use \mintinline{julia}{LinearOperators.jl}, including a LBFGS
      operator.
  \end{itemize}
\end{frame}
...
\begin{tikzpicture}
  ...
  \node[red,draw,rectangle] (ampl) at (0,-2) {AmplNLPModel};
...
\myframe{
  ...
  \begin{itemize}
    ...
    \item $\min \ f(x) = \frac{1}{2}(x_1^2 + x_2^2),$ subject to $1 \leq x_1,x_2 \leq
      2$;
...
\myframe{
  ...
  \begin{enumerate}
    \item Use $B_k = \nabla^2f(x_k)$;
    \item Use $d_k = s_k$;
    \item Trust Region is already implemented, not exactly the same, but
      similar.
  \end{enumerate}
}
...
\begin{frame}[fragile]
  \ctr{Improvements}
  \begin{itemize}
    \item Use \mintinline{julia}{LinearOperators.jl}, such as
      \mintinline{julia}{H = LinearOperator(n, Float64, v -> hprod(nlp, x, v))}
      or
      \mintinline{julia}{H = LBFGSOperator(n, mem)}
